...
import data.data64 as Data
from data.data64 import dataset2D



if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('-c', '--config', type=str, default='config/sr_sr3_256_256_condition64.json',
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train' and args.phase != 'val':
            ...
            train_set = dataset2D.create_dataset(dataset_opt, phase)
            train_loader = Data.create_dataloader(
                train_set, dataset_opt, phase)
        elif phase == 'val':
            # val_set = Data.create_dataset(dataset_opt, phase)
            val_set = dataset2D.create_dataset(dataset_opt, 'valid')
