...
class MyDataset(dataset.Dataset):
    ...
    def __getitem__(self, idx):
        NOISE_input, GT_label, xx_real, xx_imag = self.__gen_signal__(idx)

        max_amp = np.max(np.abs(NOISE_input))
        NOISE_input = np.abs(NOISE_input) / (max_amp)
        GT_label = np.abs(GT_label) / (max_amp * 5)
        xx_real = xx_real / (max_amp * 5)
        xx_imag = xx_imag / (max_amp * 5)

        return NOISE_input, GT_label, xx_real, xx_imag
