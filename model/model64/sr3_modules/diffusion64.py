...
class GaussianDiffusion(nn.Module):
    ...
    def _prepare_slice_runtime(self, x_nus):
        slice_runtime = []
        batch_meta = None
        ...
        if batch_meta:
            for raw_meta in batch_meta:
                ...
                nus_fid_full = reconstruct_data_ddpm(
                    x_nus[start:end].squeeze(1), pad_shape, pad_height, pad_width
                )
                nus_mask = torch.ne(nus_fid_full, 0)
                slice_runtime.append({
                    ...
                    'pad_width': pad_width,
                    'nus_fid_full': nus_fid_full,
                    'nus_mask': nus_mask
                })
            return slice_runtime

        if self.data_path is not None and self.data_path.get('real_data_path', None) is not None:
            ...
            nus_fid_full = reconstruct_data_ddpm(
                x_nus.squeeze(1), pad_shape, pad_height, pad_width
            )
            nus_mask = torch.ne(nus_fid_full, 0)
            slice_runtime.append({
                'start': 0,
                'end': int(x_nus.shape[0]),
    ...
    def p_sample_loop(self, x_in, nus_fid, continous=False):
        ...
        if not self.conditional:
            ...
            ret_img = x
            slice_runtime = self._prepare_slice_runtime(x_nus)
                ...
                if self.data_consistency:
                    ...
                    if slice_runtime:
                        ...
                        for runtime_meta in slice_runtime:
                            ...
                            nus_fid_full = runtime_meta['nus_fid_full']
                            nus_mask = runtime_meta['nus_mask']

                            img_slice = reconstruct_data_ddpm(
                                img_complex[start:end], pad_shape, pad_height, pad_width
                            )
                            img_slice = torch.fft.ifft2(img_slice, dim=(-2, -1))
                            img_slice[nus_mask] = (
                                                          1 * img_slice[nus_mask] + 1e3 * nus_fid_full[nus_mask]
                                                  ) / (1 + 1e3)
                            ...
                            if sparse_domain == 'freq':
                                ...
                                freq_c = torch.fft.fft2(img_slice, dim=(-2, -1))
                            ...
                            img_slice = torch.fft.fft2(fid_dc, dim=(-2, -1))
                            img_blocks, _, _, _ = block_data_ddpm(img_slice, 64)
                            updated_block_list.append(img_blocks.unsqueeze(1))

                        img = torch.cat(updated_block_list, dim=0)
